			...
			<InspectorControls>
				<div className="opry-block-sidebar">
					{venue.map((detail, index) => {
						...
						return (
							isCardOpen && (
								<PanelBody key={index} title={__(`Venue ${index + 1} Settings`, 'opry-plugin')} initialOpen={true}>
									<ToggleControl
										label={__('Show Heading', 'opry-plugin')}
									...
									/>
									<ToggleControl
										label={__('Show Title', 'opry-plugin')}
					...
					{logos.map((logo, index) => {
						...
						return (
							isLogoOpen && (
								<PanelBody key={index} title={__(`Logo ${index + 1} Settings`, 'opry-plugin')} initialOpen={true}>
									<TextControl
										type="url"
										label={__('Logo URL', 'opry-plugin')}
										placeholder={__('https://google.com', 'opry-plugin')}
										value={logo.logolink}
										onChange={value => {
											let arrayCopy = [...logos];
									...
									/>
									<ToggleControl
										label={__('Open in New Tab', 'opry-plugin')}
						...
						<PanelColorSettings
							...
							colorSettings={[
								...
								},
								{
									...
									label: __('Title Color', 'opry-plugin')
								},
								{
									value: descColor,
			...
			<div className={classes} id={blockID} style={blockStyle}>
				<div className='footer-opry-venue-info__first-section opry-footer-bottom'>
								...
								<MediaUploadCheck>
									...
									<Tooltip text={__('Remove Image', 'opry-plugin')}>
										<i
											...
											onClick={() => {
												let toDelete = confirm(__('Are you sure you want to remove this image?', 'opry-plugin'));
												if (true === toDelete) {
					...
					<RichText
						value={telNum}
						tagName="p"
						placeholder={__('Telephone Text', 'opry-plugin')}
								...
								<div key={index} className={`footer-opry-venue__item show-items-hover-wrap`}>
									<div className="item-action-wrap small-icons">
										<div className="move-item">
											{0 < index && (
												<Tooltip text={__('Move Down', 'opry-plugin')}>
													<span
											...
											{index + 1 < venue.length && (
												<Tooltip text={__('Move UP', 'opry-plugin')}>
													<span
										...
										{1 < venue.length && (
											<Tooltip text={__("Remove Item", 'opry-plugin')}>
												<i
									...
									<div className={`footer-opry-venue__card`} onClick={() => setOpenCardIndex(index)}>
										...
										{detail.showTitleText &&
											<RichText
												value={detail.title}
												tagName="p"
												placeholder={__('Enter Title', 'opry-plugin')}
												onChange={(title) => {
													let arrayCopy = [...venue];
										...
										<RichText
											value={detail.desc}
											tagName="p"
											placeholder={__('Enter Descriptions', 'opry-plugin')}
											onChange={(title) => {
												let arrayCopy = [...venue];
