		...
		<>
			<InspectorControls>
				<div className="opry-block-sidebar">
					<PanelBody title={__('Instagram Settings', 'opry-plugin')} initialOpen={true}>
						<TextControl
							type="text"
							label={__('Your Access Token', 'opry-plugin')}
							value={accessToken}
							onChange={accessToken => {
								setAttributes({ accessToken });
							}}
							help={__('Please add Instagram Access Token of Public account to view Instagram Post.', 'opry-plugin')}
						/>
					</PanelBody>
					<PanelBody title={__("Background Settings", "opry-plugin")}
						initialOpen={false}
					>
						<div className="setting-row">
							...
							{!bgImage.url ? (
								...
								<>
									...
									<MediaUploadCheck>
										<MediaUpload
											title={__("Edit Image", "opry-plugin")}
											value={bgImage.id}
											onSelect={(image) => {
												const newImage = {};
												newImage.id = image.id;
						...
						<div className="setting-row">
							<label>{__("Background Gradient", "opry-plugin")}</label>
							<GradientPicker
								value={bgGradient}
								onChange={(bgGradient) =>
				...
				<div className='container'>
					...
					<div className='insta-blog-inner__row'>
						{loading ? (
							...
							<p>{__("Invalid Access Token. Please add a valid access token to view Instagram posts.", "opry-plugin")}</p>
						) : accessToken ? (
							instaPosts ? (
								...
								<p>{__("No Instagram Posts Found", "opry-plugin")}</p>
							)
						) : (
							<p>{__("Please add access token for seeing Instagram Posts.", "opry-plugin")}</p>
						)}
					</div>
				</div>
				<div
