		...
		<Fragment>
			<InspectorControls>
				<div className="opry-block-sidebar">
					<PanelBody title={__('Background Settings', 'opry-plugin')} initialOpen={false}>
						<GradientPicker
							value={gradientColor}
							onChange={
								(value) => {
						...
						<div className="setting-row">
							<label>{__('Background Image', 'opry-plugin')}</label>
							{(!mediaURL) ? (
								<MediaUploadCheck>
									<MediaUpload
										onSelect={item => {
								...
								<Fragment>
									<div className="image-preview">
										<img src={mediaURL} alt={__('bg preview', 'opry-plugin')} />
									</div>
									<MediaUploadCheck>
										<MediaUpload
											onSelect={item => {
						...
						<div className="setting-row">
							<SelectControl
								label={__('Background Position', 'opry-plugin')}
								value={bgPosition}
					...
					<PanelBody title={__('Color Settings', 'opry-plugin')} initialOpen={false}>
						<PanelColorSettings
							title={__('Colors', 'opry-plugin')}
							colorSettings={[
								{
									value: bgColor,
									onChange: value => { setAttributes({ bgColor: value }) },
									label: __('Background Color', 'opry-plugin')
								},
								{
									value: headingColor,
									onChange: value => { setAttributes({ headingColor: value }) },
									label: __('Heading Color', 'opry-plugin')
								},
								{
									value: descColor,
									onChange: value => { setAttributes({ descColor: value }) },
									label: __('Description Color', 'opry-plugin')
								}
							]}
						/>
					</PanelBody>
					<PanelBody title={__('Audio Settings', 'opry-plugin')} initialOpen={false}>
						<div className="setting-row">
							<label>{__('Audio File', 'opry-plugin')}</label>
							{(!audioURL) ? (
								<MediaUploadCheck>
									<MediaUpload
										onSelect={item => {
					...
					<div className='footer-audio-inner'>
						<div className='footer-audio-inner__first-section'>
							<RichText
								...
								placeholder={__('Enter Heading', 'opry-plugin')}
								onChange={(heading) => {
							...
							<RichText
								...
								placeholder={__('Enter Description', 'opry-plugin')}
								onChange={(desc) => {
						...
						<div className='footer-audio-inner__second-section'>
							<div className='footer-audio-player'>
								...
								<Button onClick={toggleAudio}
									...
									title={isPlaying ? __('Pause WSM Radio', 'opry-plugin') : __('Play WSM Radio', 'opry-plugin')}
									aria-label={isPlaying ? __('Pause WSM Radio', 'opry-plugin') : __('Play WSM Radio', 'opry-plugin')}
								...
								>{playIcon}
									<RichText
										value={audioText}
										tagName="p"
										placeholder={__('Audio Text', 'opry-plugin')}
							...
							<div className='footer-audio-inner__btn-section'>
								<RichText
									value={btnText}
									tagName="p"
									placeholder={__('Button Text', 'opry-plugin')}
									...
									<div className="image-controls small-icons icon-center-fixed">
										<MediaUploadCheck>
											<MediaUpload
												...
												render={({ open }) => {
													return (
														<Tooltip text={__('Edit Image', 'opry-plugin')}>
															<i
											...
											/>
											<Tooltip text={__('Remove Image', 'opry-plugin')}>
												<i
