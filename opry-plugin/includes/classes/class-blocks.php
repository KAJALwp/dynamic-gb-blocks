...
class Blocks {
	...
	public function register_blocks(): void {
		...
		if ( file_exists( OPRY_PLUGIN_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( OPRY_PLUGIN_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	...
	public function custom_block_category( array $categories ): array {
		return array_merge(
			array(
				array(
					'slug'  => 'opry-plugin',
					'title' => __( 'Opry Plugin', 'opry-plugin' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'opry-plugin/' ) ) {
			$string = str_replace( 'opry-plugin/', '', $string );
		}
	...
	public function localize_gutenberg_blocks_config(): void {
		$blocks_config = apply_filters( 'opry_gutenberg_blocks_config', array() );

		wp_localize_script(
			'wp-edit-post',
			'opry_gutenberg_blocks_config',
			$blocks_config
		);
	}
}
